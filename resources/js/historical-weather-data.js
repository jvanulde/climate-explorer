...
$(function () {
  ...
  var mode = 'daily_vs_climate'; // stationsMapState['mode'];
  ...
  if (stationId) {
    ...
    // reset graphs
    resetGraphs({ variable: 'temperature', stationId: stationId, stationName: stationName });;
  ...
  $('.download-select li a').click(function (e) {
    ...
    switch (downloadAction) {
      case 'download-precipitation-image':
        // download image
  ...
  $('#stations-select-vis').bind('cs-changed', function (e) {
    ...
    if (notDisabled) {
      ...
      var _stationName = val[1];
      var _stationId = val[0];
      ...
      updateStationIDText('' + _stationId);
      updateStationText('' + _stationName);

      // change map varriable
      window.ce.ce('setStationsMapState', { stationId: _stationId, stationName: _stationName });
      ...
      // reset graphs
      resetGraphs({ variable: 'temperature', stationId: _stationId, stationName: _stationName });;
  ...
  $('#filters-toggle').click(function (e) {
    ...
    var stationsMapRowElem = $('#stations-map-wrap');
  ...
  window.stations = $('#stations-map').stationsMap(_extends({
    ...
    change: function change(event, options) {
      ...
      if (options.currentstations.features.length === 0) {
        ...
        if (messsageElem) {
          var rect = document.getElementById('stations-map').getBoundingClientRect();
  ...
  function setMapSize() {
    ...
    var graphWidth = graphRect.width / 2;
