...
$(function () {
  ...
  $('#varriable-select-vis').bind('cs-changed', function (e) {
    var variable = $('#varriable-select-vis').attr('rel');
    // update the chart based on char varriable
    window.tempChart.update({
      variable: variable
    });

    window.ce.ce('setVariablesMapState', { variable: variable });

    // update the text
    updateTitle($('#varriable-select-vis').text());

    // disable varriablles if they are valid time period
    var isvalid = jQuery.inArray(variable, validMonthly);
    if (isvalid < 0) {
  ...
  var variable = window.ce.ce('getVariablesPageState')['variable'];
