  ...
  $.widget('ce.scenarioComparisonMap', {
    ...
    _initMap: function _initMap() {
      ...
      this.dojoMods.watchUtils.whenTrue(this.view, "stationary", function () {
        if (this.view.center) {
          var latlon = this.dojoMods.webMercatorUtils.xyToLngLat(this.view.center.x, this.view.center.y);
    ...
    _updateLeftScenarioLayer: function _updateLeftScenarioLayer() {
      if (this.leftLayer !== null) {
        ...
        var lay = this.map.findLayerById(this.leftLayer.id);
        this.map.remove(lay);
        // setTimeout(function () {
    ...
    _updateRightScenarioLayer: function _updateRightScenarioLayer() {
      if (this.rightLayer !== null) {
        ...
        var lay = this.map.findLayerById(this.rightLayer.id);
        this.map.remove(lay);
        // setTimeout(function () {
    ...
    _initScenarioLayer: function _initScenarioLayer(scenario, year, side, opacity) {
      var promise = new Promise(function (resolve) {
        ...
        if (this.options.scenarios[scenario].max_zoom_level && this.options.scenarios[scenario].min_zoom_level) {
          layer.tileInfo.lods = layer.tileInfo.lods.filter(function (lod) {
            return lod.level >= _this9.options.scenarios[scenario].min_zoom_level && lod.level <= _this9.options.scenarios[scenario].max_zoom_level;
          });
        }
        ...
        this._mapInitPromise.then(function () {
          ...
          this.view.on("click", function (event) {
            this.countiesLayer.queryFeatures({ geometry: event.mapPoint, outFields: ["*"] }).then(function (response) {
              ...
              var link = document.querySelector('#national-climate-maps-secretlink-forcharts');
              var params = '?fips=' + this.options.county + '&county=' + this.options.countyName;

              // set the url and search params
              var url = $(link).attr('href') + '/?fips=' + this.options.county + '&lat=' + event.mapPoint.latitude + '&lon=' + event.mapPoint.longitude + '&city=' + this.options.countyName + '  County, ' + this.options.stateName + ' County&county=' + this.options.countyName + ' County&nav=local-climate-charts';
              $(link).attr('href', url);
    ...
    _getScenarioURL: function _getScenarioURL(scenario, year) {
      ...
      tilesURL = tilesURL.replace('{scenario}', scenario).replace('{season}', season).replace('{season_month}', season_month).replace('{variable}', variable).replace('{year}', year).replace(/\/\//g, function (i) {
        return function (m) {
          return !i++ ? m : '/';
        };
      }(0));
